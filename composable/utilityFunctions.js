...
export default function genericFuncs() {
    ...
    function updateReport(jobid, reportType, data) {
        return new Promise((resolve, reject) => {
            firebase.auth().currentUser.getIdToken().then((idToken) => {
                axios.post(`${process.env.serverUrl}/api/report/${reportType}/${jobid}/update`, data, {headers: {authorization: `Bearer ${idToken}`}}).then((res) => {
                    resolve(res)
                }).catch(err => {
                    reject(err)
                })
